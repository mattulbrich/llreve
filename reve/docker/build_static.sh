#!/bin/bash
# currently we assume that an image named cocreature/llreve:llvm-5.0_z3-4.5 already exists
set -o nounset
set -o errexit
...
BUILD_COMMAND='rm -rf /llreve/reve/build && \
               ...
               cmake .. -GNinja -DCMAKE_BUILD_TYPE=Release \
                                -DLLREVE_STATIC=ON \
               ninja'
CONTAINER="$(docker create cocreature/llreve:llvm-5.0_z3-4.5 sh -c "${BUILD_COMMAND}")"
function finish {
    docker rm "${CONTAINER}"
}
trap finish EXIT
