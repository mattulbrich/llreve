...
#ifndef LLREVE_SUPPORT_COMMANDLINE_H
#define LLREVE_SUPPORT_COMMANDLINE_H
...
enum NumOccurrencesFlag { // Flags for the number of occurrences allowed
    ...
    // ConsumeAfter - Indicates that this option is fed anything that follows
    // the
...
extern llvm::ManagedStatic<SubCommand> TopLevelSubCommand;

// A special subcommand that can be used to put an option into all subcommands.
extern llvm::ManagedStatic<SubCommand> AllSubCommands;
...
class Option {
    ...
    //
    virtual bool handleOccurrence(unsigned pos, llvm::StringRef ArgName,
                                  llvm::StringRef Arg) = 0;
  ...
  public:
    ...
    bool isInAllSubCommands() const {
        return any_of(
            Subs, [](const SubCommand *SC) { return SC == &*AllSubCommands; });
    }
  ...
  protected:
    explicit Option(enum NumOccurrencesFlag OccurrencesFlag,
        ...
        : NumOccurrences(0), Occurrences(OccurrencesFlag), Value(0),
          ...
          Position(0), AdditionalVals(0), ArgStr(""), HelpStr(""), ValueStr(""),
  ...
  public:
    ...
    //
    virtual bool addOccurrence(unsigned pos, llvm::StringRef ArgName,
                               llvm::StringRef Value, bool MultiArg = false);
...
struct OptionValue<std::string> final : OptionValueCopy<std::string> {
    typedef llvm::StringRef WrapperType;
...
class generic_parser_base {
  ...
  public:
    ...
    void
    getExtraOptionNames(llvm::SmallVectorImpl<llvm::StringRef> &OptionNames) {
        // If there has been no argstr specified, that means that we need to add
        // an
        // argument for every possible option.  This ensures that our options
        // are
        // vectored to us.
        if (!Owner.hasArgStr())
    ...
    enum ValueExpected getValueExpectedFlagDefault() const {
        ...
        //
        // In which case, the value is required.  Otherwise if an arg str has
        // not
    ...
    // argument string.  If the option is not found, getNumOptions() is
    // returned.
    //
...
class basic_parser_impl { // non-template implementation of basic_parser<t>
  public:
    ...
    // printOptionNoValue - Print a placeholder for options that don't yet
    // support
    // printOptionDiff().
...
template <unsigned n> struct applicator<char[n]> {
    template <class Opt> static void opt(llvm::StringRef Str, Opt &O) {
        O.setArgStr(Str);
    }
};
...
template <> struct applicator<llvm::StringRef> {
    template <class Opt> static void opt(llvm::StringRef Str, Opt &O) {
        O.setArgStr(Str);
    }
};
...
template <class DataType> class list_storage<DataType, bool> {
  ...
  public:
    ...
    operator llvm::ArrayRef<DataType>() { return Storage; }
...
llvm::StringMap<Option *> &
getRegisteredOptions(SubCommand &Sub = *TopLevelSubCommand);
...
typedef void (*TokenizerCallback)(llvm::StringRef Source,
                                  llvm::StringSaver &Saver,
                                  llvm::SmallVectorImpl<const char *> &NewArgv,
                                  bool MarkEOLs);
...
/// not specific to the tool. This function allows a tool to specify a single
/// option category to display in the -help output.
void HideUnrelatedOptions(llvm::ArrayRef<const cl::OptionCategory *> Categories,
                          SubCommand &Sub = *TopLevelSubCommand);
