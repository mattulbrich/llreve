// These files are copied from LLVM because since LLVM 3.9
// using this API interferes with the Clang API.
// By changing the namespace we can avoid this.
...
OptionCategory llreve::cl::GeneralCategory("General options");
...
ManagedStatic<SubCommand> llreve::cl::TopLevelSubCommand;

// A special subcommand that can be used to put an option into all subcommands.
ManagedStatic<SubCommand> llreve::cl::AllSubCommands;
...
static Option *LookupNearestOption(StringRef Arg,
    ...
    for (StringMap<Option *>::const_iterator it = OptionsMap.begin(),
        ...
        for (auto Name : OptionNames) {
            unsigned Distance =
                StringRef(Name).edit_distance(Flag, /*AllowReplacements=*/true,
...
HandlePrefixedOrGroupedOption(StringRef &Arg, StringRef &Value,
    ...
    size_t Length = 0;
    Option *PGOpt =
        getOptionPred(Arg, Length, isPrefixedOrGrouping, OptionsMap);
    if (!PGOpt)
        return nullptr;
...
bool cl::ExpandResponseFiles(StringSaver &Saver, TokenizerCallback Tokenizer,
    ...
    for (unsigned I = 0; I != Argv.size();) {
        ...
        // iterations.
        // FIXME: If a nested response file uses a relative path, is it relative
        // to
...
bool CommandLineParser::ParseCommandLineOptions(int argc,
        ...
        for (size_t i = 0, e = PositionalOpts.size(); i != e; ++i) {
            ...
            if (RequiresValue(Opt))
                ...
                if (PositionalOpts.size() > 1) {
                    if (!IgnoreErrors)
                        Opt->error("error - this positional option will never "
                                   "be matched, "
    ...
    for (int i = FirstArg; i < argc; ++i) {
        ...
        if (argv[i][0] != '-' || argv[i][1] == 0 || DashDashFound) {
            ...
            if (ActivePositionalArg) {
                ProvidePositionalOption(ActivePositionalArg, StringRef(argv[i]),
                                        i);
            ...
            if (!PositionalOpts.empty()) {
                ...
                // All of the positional arguments have been fulfulled, give the
                // rest to
                ...
                if (PositionalVals.size() >= NumPositionalRequired &&
                    ...
                    for (++i; i < argc; ++i)
                        PositionalVals.push_back(
                            std::make_pair(StringRef(argv[i]), i));
            ...
            // this
            // option is another positional argument.  If so, treat it as an
            // argument,
            ...
            // argument.
            if (!Handler)
                Handler = HandlePrefixedOrGroupedOption(
                    ArgName, Value, ErrorParsing, OptionsMap);

            // Otherwise, look for the closest available option to report to the
            // user
            // in the upcoming error.
            if (!Handler && SinkOpts.empty())
                NearestHandler = LookupNearestOption(ArgName, OptionsMap,
    ...
    if (NumPositionalRequired > PositionalVals.size()) {
        ...
        for (size_t i = 0, e = PositionalOpts.size(); i != e; ++i) {
            ...
            // If we _can_ give this option more arguments, do so now, as long
            // as we
            // do not give it values that others need.  'Done' controls whether
            // the
            // option even _WANTS_ any more.
            //
            bool Done =
                PositionalOpts[i]->getNumOccurrencesFlag() == cl::Required;
            while (NumVals - ValNo > NumPositionalRequired && !Done) {
                switch (PositionalOpts[i]->getNumOccurrencesFlag()) {
                case cl::Optional:
                    Done = true; // Optional arguments want _at most_ one value
                case cl::ZeroOrMore: // Zero or more will take all they can
                                     // get...
        ...
        assert(ConsumeAfterOpt &&
               NumPositionalRequired <= PositionalVals.size());
        unsigned ValNo = 0;
        for (size_t j = 1, e = PositionalOpts.size(); j != e; ++j)
            if (RequiresValue(PositionalOpts[j])) {
                ErrorParsing |= ProvidePositionalOption(
                    PositionalOpts[j], PositionalVals[ValNo].first,
                    PositionalVals[ValNo].second);
                ValNo++;
            }
        ...
        // loop would have assigned no values to positional options in this
        // case.
        //
        if (PositionalOpts.size() == 1 && ValNo == 0 &&
            !PositionalVals.empty()) {
            ErrorParsing |= ProvidePositionalOption(
                PositionalOpts[0], PositionalVals[ValNo].first,
                PositionalVals[ValNo].second);
            ValNo++;
        }
        ...
        for (; ValNo != PositionalVals.size(); ++ValNo)
            ErrorParsing |= ProvidePositionalOption(
                ConsumeAfterOpt, PositionalVals[ValNo].first,
                PositionalVals[ValNo].second);
    }
...
class CategorizedHelpPrinter : public HelpPrinter {
  ...
  protected:
    void printOptions(StrOptionPairVector &Opts, size_t MaxArgLen) override {
        ...
        assert(SortedCategories.size() > 0 &&
...
void HelpPrinterWrapper::operator=(bool Value) {
    ...
    if (GlobalParser->RegisteredOptionCategories.size() > 1) {
        // unhide -help-list option so user can have uncategorized output if
        // they
        // want it.
...
class VersionPrinter {
  public:
    ...
    void operator=(bool OptionWasSpecified) {
        ...
        // Iterate over any registered extra printers and call them to add
        // further
        // information.
        if (ExtraVersionPrinters != nullptr) {
            outs() << '\n';
            for (std::vector<void (*)()>::iterator
                     I = ExtraVersionPrinters->begin(),
...
void cl::PrintHelpMessage(bool Hidden, bool Categorized) {
    ...
    // assigned true/false depending on whether -help or -help-hidden was given
    // or
    // not.  Since we're circumventing that we have to make it look like -help
    // or
